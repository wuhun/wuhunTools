package tools.com.lvliangliang.wuhuntools.widget.recyclerview;
 ...
 */
public class WuhunRecyclerView extends RecyclerView {
    ...
    public WuhunRecyclerView(Context context) {
        super(context, null);
    }

    public WuhunRecyclerView(Context context, AttributeSet attrs) {
        super(context, attrs);

        mContext = context;

        /*================== 获取自定义属性 begin ==================*/
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.WuhunRecyclerView);
        for (int i = 0; i < typedArray.getIndexCount(); i++) {
            int attr = typedArray.getIndex(i);
            if (attr == R.styleable.WuhunRecyclerView_rv_type) {
                type = typedArray.getInt(attr, 0);
            } else if (attr == R.styleable.WuhunRecyclerView_rv_orientation) {
                orientation = typedArray.getInt(attr, 0);
            } else if (attr == R.styleable.WuhunRecyclerView_rv_column) {
                column = typedArray.getInt(attr, 1);
            } else if (attr == R.styleable.WuhunRecyclerView_rv_divider_size) {
                dividerSize = (int) typedArray.getDimension(attr, 0f);
            } else if (attr == R.styleable.WuhunRecyclerView_rv_divider_drawable) {
                dividerDrawable = typedArray.getDrawable(attr);
            } else if (attr == R.styleable.WuhunRecyclerView_rv_divider_color) {
                dividerColor = typedArray.getColor(attr, Color.BLACK);
            } else if (attr == R.styleable.WuhunRecyclerView_rv_default_animator_open) {
    ...
    class RecyclerViewListener extends RecyclerView.OnScrollListener {
        ...
        public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
            ...
            if (move && newState == RecyclerView.SCROLL_STATE_IDLE) {
                ...
                if (0 <= n && n < WuhunRecyclerView.this.getChildCount()) {
                    int top = WuhunRecyclerView.this.getChildAt(n).getTop();
                    WuhunRecyclerView.this.smoothScrollBy(0, top);
                }
        ...
        public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
            ...
            if (move) {
                ...
                if (0 <= n && n < WuhunRecyclerView.this.getChildCount()) {
                    int top = WuhunRecyclerView.this.getChildAt(n).getTop();
                    WuhunRecyclerView.this.scrollBy(0, top);
                }
            }
        }
    }
    ...
    class LQRItemDecoration extends ItemDecoration {
        ...
        public LQRItemDecoration(Context context, int orientation, int dividerSize, int dividerColor, Drawable dividerDrawable) {
            ...
            if (dividerDrawable == null) {
                ...
                if (mDividerSize == 0) {
                    if (mOrientation == WuhunRecyclerView.ORIENTATION_VERTICAL) {
